...
let player23;
let player24;
var boopBtn = document.getElementById('logo');

function setupSynth() {
  let player23 = new Tone.Player({
    url: "https://assets.codepen.io/2173169/name_bf.wav",
    autostart: true,
    loop: false
  }).toDestination();
  let player24 = new Tone.Player({
    url: "https://assets.codepen.io/2173169/name_ef.wav",
    autostart: true,
    loop: false
  }).toDestination();
}
...
boopBtn.addEventListener('touchstart', function(e) {
  e.stopPropagation();
  e.preventDefault();
  boop();
});

boopBtn.addEventListener('mousedown', boop);
...
function render() {
  ...
  if (vol > 1 / 18 * 8 && vol < 1 / 18 * 17) {
    ...
    player23.volume.value = -Infinity;
    player24.volume.value = 0;
    ...
    player23.volume.value = 0;
    player24.volume.value = -Infinity;
  }

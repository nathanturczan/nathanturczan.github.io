...
function pick_scale(key) {
    ...
    function on_midi_success(midi) {
        ...
        midi.outputs.forEach(function (port, port_id) {
            ...
            port.send([0b10010000, scales[key].pitch_classes[0], 127]);
            port.send([0b10010000, scales[key].pitch_classes[0]+12, 127]);
            port.send([0b10010000, scales[key].pitch_classes[0]+24, 127]);
            port.send([0b10010000, scales[key].pitch_classes[0]+36, 127]);
            port.send([0b10010000, scales[key].pitch_classes[0]+48, 127]);
            port.send([0b10010000, scales[key].pitch_classes[4]+48, 127]);
            port.send([0b10010000, scales[key].pitch_classes[0]+60, 127]);
            port.send([0b10010000, scales[key].pitch_classes[2]+60, 127]);
            port.send([0b10010000, scales[key].pitch_classes[4]+60, 127]);
            port.send([0b10010000, scales[key].pitch_classes[6]+60, 127]);
            port.send([0b10010000, scales[key].pitch_classes[0]+72, 127]);
            port.send([0b10010000, scales[key].pitch_classes[1]+72, 127]);
            port.send([0b10010000, scales[key].pitch_classes[2]+72, 127]);
            port.send([0b10010000, scales[key].pitch_classes[3]+72, 127]);
            port.send([0b10010000, scales[key].pitch_classes[4]+72, 127]);
            port.send([0b10010000, scales[key].pitch_classes[5]+72, 127]);
            port.send([0b10010000, scales[key].pitch_classes[6]+72, 127]);
            port.send([0b10010000, scales[key].pitch_classes[0]+84, 127]);
            if (onkeyup > 0 ){
                port.send([0b10010000, 0, 0]);
            }
        });
    }
...
function polygon(x, y, radius, npoints) {
  ...
  for ( let a = 0; a < TWO_PI; a += angle) {
    ...
    if (npoints == 4 ) {
        let sx = x + cos(a+(TWO_PI/8)) * radius;
        let sy = y + sin(a+(TWO_PI/8)) * radius;
        vertex(sx, sy);
    }
    if (npoints == 5 ) {
...
function drawScale(key, x, y, level, ancestors, offset) {
    ...
    fill(hsvToRgb(map(scales[key].root, 11, 0, 0, 1), map(scales[key].root, 0, 11, 0.5, 1), map(level, 0, 4, 0.95, 1)));
    ...
    polygon(x, y, shape_size, scales[key].adjacent_scales.length);
  
    stroke(0);
    fill(0);
    const font_size = 30/level;
    textSize(font_size);
    text(key, x-(80 / level), y);
    //use symbols instead of printing out EVERYTHING
    //text()
    text(scales[key].scale_class, x-(49 / level), y+(33 / level)); //print out scale class
    ...
    for( let i = 0; i < filt_adjacent_scales.length; i++ ) {
        ...
        stroke(hsvToRgb(map(scales[key].root, 11, 0, 0, 1), map(scales[key].root, 0, 11, 0.5, 1), map(level, 0, 4, 0.95, 1)));
        line(x, y, newX, newY);
        stroke(hsvToRgb(map(scales[key].root, 11, 0, 1, 0), map(scales[key].root, 0, 11, 0.5, 1), map(level, 0, 4, 0, 1)));
    }
}
